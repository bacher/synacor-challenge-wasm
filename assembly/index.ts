...
import { functionsOffsets, labelsOffsets, reportOp } from './debug';
...
const dynamicCalls: StaticArray<u16> = [749, 1520, 2790, 2972, 3619];
...
function nextOp(): VmReturnType {
  ...
  switch (opCode) {
    ...
    case 17: {
      ...
      if (debugMode) {
        if (opOffset === 1520 && a === 1553) {
          break;
        }
        if (opOffset === 1558 && a === 2147) {
          break;
        }

        const modifier = dynamicCalls.includes(opOffset) ? ' (dynamic)' : '';
